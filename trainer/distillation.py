...
from utils.dataset import TextDataset, TwoTextDataset, ThreeTextDataset, cycle
...
from model import DMD, DMDSwitch, DMDCallback
...
# from one_logger_utils import OneLoggerUtils  # Disabled
import time
...
class Trainer:
    ...
    def __init__(self, config):
        ...
        self.use_one_logger = False  # Disabled: getattr(config, "use_one_logger", True)
        ...
        # app_start_time = time.time_ns() / 1_000_000  # Disabled
        
        # ------------------------------------- One Logger Setup ----------------------------------------------
        # Disabled: One Logger initialization
        ...
        self.one_logger = None  # Disabled

        # Step 2: Initialize the model
        # Disabled: if self.one_logger is not None:
        ...
        elif config.distribution_loss == "dmd_callback":
            self.model = DMDCallback(config, device=self.device)
        ...
        self.critic_base_loaded = True  # Default: assume critic base weights are available
        if hasattr(config, 'adapter') and config.adapter is not None:
            ...
            if base_checkpoint_path:
                ...
                # Load critic base weights
                # Priority: 1. critic_ckpt (separate file) 2. critic in base_checkpoint 3. default pretrained
                self.critic_base_loaded = False
                critic_ckpt_path = getattr(config, "critic_ckpt", None)
                
                if critic_ckpt_path:
                    # Load from separate critic checkpoint file
                    if self.is_main_process:
                        print(f"Loading critic base weights from separate file: {critic_ckpt_path}")
                    critic_checkpoint = torch.load(critic_ckpt_path, map_location="cpu")
                    
                    # Handle different checkpoint formats
                    if "critic" in critic_checkpoint:
                        self.model.fake_score.load_state_dict(critic_checkpoint["critic"], strict=True)
                    elif "model" in critic_checkpoint:
                        self.model.fake_score.load_state_dict(critic_checkpoint["model"], strict=True)
                    else:
                        # Assume the checkpoint is the state dict itself
                        self.model.fake_score.load_state_dict(critic_checkpoint, strict=True)
                    
                    self.critic_base_loaded = True
                    if self.is_main_process:
                        print("Critic base weights loaded successfully from separate file")
                        
                elif "critic" in base_checkpoint:
                    # Load from generator checkpoint (combined checkpoint)
                    if self.is_main_process:
                        print(f"Loading finetuned critic from {base_checkpoint_path}")
                    self.model.fake_score.load_state_dict(base_checkpoint["critic"], strict=True)
                    self.critic_base_loaded = True
                    if self.is_main_process:
                        print("Critic weights loaded successfully from base checkpoint")
                else:
                    if self.is_main_process:
                        print(f"Note: Critic checkpoint not found. "
                              f"Critic will use default pretrained weights from '{self.model.fake_model_name}'.")
            else:
                if self.is_main_process:
                    raise ValueError("No base model checkpoint specified for LoRA training.")
            
            # Load training step (unless reset_step is set)
            reset_step = getattr(config, "reset_step", False)
            if reset_step:
                self.step = 0
                if self.is_main_process:
                    print(f"reset_step=True: Starting from step 0 (ignoring base checkpoint step)")
            elif "step" in base_checkpoint:
            ...
            # 2. (Optional) Merge previous stage LoRA into base model before applying new LoRA
            # This prevents catastrophic forgetting by "baking in" previous stage abilities
            merge_lora_before_training = getattr(config, "merge_lora_before_training", False)
            lora_to_merge_path = getattr(config, "lora_ckpt_to_merge", None) or getattr(config, "lora_ckpt", None)
            
            if merge_lora_before_training and lora_to_merge_path:
                if self.is_main_process:
                    print(f"\n{'='*60}")
                    print("MERGE MODE: Merging previous stage LoRA into base model")
                    print(f"{'='*60}")
                    print(f"LoRA checkpoint: {lora_to_merge_path}")
                
                # Load previous LoRA checkpoint
                prev_lora_ckpt = torch.load(lora_to_merge_path, map_location="cpu")
                if "generator_lora" not in prev_lora_ckpt:
                    raise ValueError(f"Cannot merge: {lora_to_merge_path} does not contain 'generator_lora' key")
                
                # Get LoRA config for merging - priority:
                # 1. checkpoint's lora_config (if saved during training)
                # 2. lora_ckpt_to_merge_config (manually specified in yaml)
                # 3. current adapter config (fallback)
                if "lora_config" in prev_lora_ckpt:
                    prev_lora_config = prev_lora_ckpt["lora_config"]
                    lora_rank = prev_lora_config.get("rank", 256)
                    lora_alpha = prev_lora_config.get("alpha", 256)
                    if self.is_main_process:
                        print(f"Using lora_config from checkpoint: rank={lora_rank}, alpha={lora_alpha}")
                elif hasattr(config, "lora_ckpt_to_merge_config") and config.lora_ckpt_to_merge_config:
                    merge_config = config.lora_ckpt_to_merge_config
                    lora_rank = merge_config.get("rank", 256)
                    lora_alpha = merge_config.get("alpha", 256)
                    if self.is_main_process:
                        print(f"Using lora_ckpt_to_merge_config: rank={lora_rank}, alpha={lora_alpha}")
                else:
                    lora_rank = getattr(self.lora_config, 'rank', 256)
                    lora_alpha = getattr(self.lora_config, 'alpha', 256)
                    if self.is_main_process:
                        print(f"Warning: No lora_config in checkpoint or yaml, using current config: rank={lora_rank}, alpha={lora_alpha}")
                
                scaling = lora_alpha / lora_rank
                
                if self.is_main_process:
                    print(f"Merge scaling factor: {scaling}")
                    print(f"\n--- Merging Generator LoRA ---")
                
                # Merge generator LoRA weights into base model
                merged_count = self._merge_lora_into_base(
                    self.model.generator.model,
                    prev_lora_ckpt["generator_lora"],
                    scaling=scaling
                )
                
                if self.is_main_process:
                    print(f"Generator: merged {merged_count} LoRA weight pairs")
                
                # Merge critic LoRA (if available and critic LoRA is enabled)
                if getattr(self.lora_config, 'apply_to_critic', True):
                    if "critic_lora" in prev_lora_ckpt:
                        if self.is_main_process:
                            print(f"\n--- Merging Critic LoRA ---")
                        
                        critic_merged = self._merge_lora_into_base(
                            self.model.fake_score.model,
                            prev_lora_ckpt["critic_lora"],
                            scaling=scaling
                        )
                        
                        if self.is_main_process:
                            print(f"Critic: merged {critic_merged} LoRA weight pairs")
                    else:
                        if self.is_main_process:
                            print(f"\n--- Critic LoRA ---")
                            print(f"Warning: 'critic_lora' not found in {lora_to_merge_path}")
                            print(f"Critic will use base weights without LoRA merge")
                
                if self.is_main_process:
                    print(f"\n--- Merge Summary ---")
                    print(f"Previous stage abilities are now frozen in base model")
                    print(f"Fresh LoRA will be initialized for this training stage")
                    print(f"{'='*60}\n")
            
            # 3. Apply LoRA wrapping now (after loading base model, before FSDP wrapping)
            if self.is_main_process:
            ...
            if auto_resume and self.output_path:
                ...
                if latest_checkpoint:
                    try:
                        ...
                        # Only require generator_lora; critic_lora is optional
                        if "generator_lora" in checkpoint:
                            ...
                            if self.is_main_process:
                                ...
                                if "critic_lora" not in checkpoint:
                                    print("  Note: checkpoint does not contain critic_lora weights")
                        else:
                            raise ValueError(f"Checkpoint {latest_checkpoint} is not a valid LoRA checkpoint. "
                                           f"Expected 'generator_lora' key, found keys: {list(checkpoint.keys())}")
            ...
            if lora_checkpoint_path:
                ...
                # Only load critic LoRA if critic base weights from checkpoint were loaded
                # If critic uses default pretrained weights, we should not load old LoRA weights
                # because they were trained on a different critic state
                if "critic_lora" in lora_checkpoint:
                    if getattr(self, 'critic_base_loaded', True):
                        if self.is_main_process:
                        ...
                        if self.is_main_process:
                            print(f"Skipping critic LoRA weights from checkpoint: critic base weights were not loaded from checkpoint. "
                                  f"Critic will use default pretrained weights with freshly initialized LoRA parameters.")
                else:
                    if self.is_main_process:
                        print("No critic_lora in checkpoint, critic LoRA parameters will be randomly initialized")
                ...
                if reset_step:
                    self.step = 0
                    if self.is_main_process:
                        print(f"reset_step=True: Starting from step 0 (ignoring checkpoint step)")
                elif "step" in lora_checkpoint:
        ...
        elif self.config.distribution_loss == "dmd_callback":
            # Support both three-prompt (callback) and two-prompt (switch-only with memory) modes
            if hasattr(config, 'third_prompt_path') and config.third_prompt_path:
                dataset = ThreeTextDataset(config.data_path, config.switch_prompt_path, config.third_prompt_path)
            else:
        ...
        if self.vis_interval > 0 and len(getattr(config, "vis_video_lengths", [])) > 0:
            ...
            if self.config.i2v:
                ...
                # Support both three-prompt (callback) and two-prompt (switch-only with memory) modes
                if hasattr(config, 'val_third_prompt_path') and config.val_third_prompt_path:
                    val_dataset = ThreeTextDataset(val_data_path, config.val_switch_prompt_path, config.val_third_prompt_path)
                else:
    ...
    def _log_selected_prompts(self, batch, switch_frame_index):
        """Append selected prompts to text files in logdir for reproducibility."""
        if not self.output_path:
            return
        os.makedirs(self.output_path, exist_ok=True)

        prompt_file = os.path.join(self.output_path, "selected_prompt.txt")
        switch_file = os.path.join(self.output_path, "selected_switch_prompt.txt")

        # Extract the first prompt
        if "switch_prompts0" in batch:
            prompt_text = batch["switch_prompts0"]
        ...
        # Extract the switch prompt(s)
        switch_texts = []
        if "switch_prompts1" in batch:
            t = batch["switch_prompts1"]
            switch_texts.append(t[0] if isinstance(t, (list, tuple)) else t)
        if "switch_prompts2" in batch:
            t = batch["switch_prompts2"]
            switch_texts.append(t[0] if isinstance(t, (list, tuple)) else t)
        if not switch_texts and "switch_prompts" in batch:
            t = batch["switch_prompts"]
            switch_texts.append(t[0] if isinstance(t, (list, tuple)) else t)

        sw_idx = switch_frame_index if switch_frame_index is not None else -1

        with open(prompt_file, "a", encoding="utf-8") as f:
            f.write(f"step={self.step}\tswitch_at={sw_idx}\t{prompt_text}\n")

        with open(switch_file, "a", encoding="utf-8") as f:
            for i, st in enumerate(switch_texts):
                f.write(f"step={self.step}\tswitch_at={sw_idx}\tsegment={i+1}\t{st}\n")
    ...
    def save(self):
        ...
        # Disabled: if getattr(self, 'one_logger', None) is not None and self.is_main_process:
        ...
        if self.is_lora_enabled:
            ...
            # Save lora_config so that inference/next stage can use correct scaling
            lora_config_dict = {
                "type": getattr(self.lora_config, 'type', 'lora'),
                "rank": getattr(self.lora_config, 'rank', 256),
                "alpha": getattr(self.lora_config, 'alpha', 256),
                "dropout": getattr(self.lora_config, 'dropout', 0.0),
                "dtype": getattr(self.lora_config, 'dtype', 'bfloat16'),
                "apply_to_critic": getattr(self.lora_config, 'apply_to_critic', True),
                "verbose": getattr(self.lora_config, 'verbose', False),
            }
            state_dict = {
    ...
    def generate_video_with_multi_prompt_switch(
        self,
        pipeline,
        num_frames,
        text_prompts_list,
        ...
        batch_size = len(text_prompts_list[0])
        if image is not None and self.is_main_process:
            print("[Warning] i2v visualization is not supported in InteractiveMemoryCausalInferencePipelineEB; using noise-only generation.")
    ...
    def start_new_sequence(self):
        ...
        # For callback training, ThreeTextDataset uses switch_prompts0/1/2
        # For switch training, TwoTextDataset uses prompts/switch_prompts
        # For regular training, TextDataset uses prompts
        if "switch_prompts0" in batch:
            # ThreeTextDataset (callback training)
            text_prompts = batch["switch_prompts0"]
        else:
            # TwoTextDataset or TextDataset
            text_prompts = batch["prompts"]
        if self.config.i2v:
        ...
        # Handle DMD Switch/Callback related information
        switch_conditional_dict = None
        switch_frame_index = None
        callback_conditional_dict = None
        callback_frame_index = None
        
        # Handle DMDCallback (three prompts)
        if isinstance(self.model, DMDCallback) and "switch_prompts1" in batch:
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                print(f"[SeqTrain-Trainer] Processing DMDCallback info (three prompts)")
            
            # Note: ThreeTextDataset returns switch_prompts0, switch_prompts1, switch_prompts2
            # switch_prompts0 is the first prompt (used as conditional_dict above via text_prompts alias)
            with torch.no_grad():
                # Second segment prompt
                switch_conditional_dict = self.model.text_encoder(
                    text_prompts=batch["switch_prompts1"]
                )
                # Callback (third) segment prompt
                callback_conditional_dict = self.model.text_encoder(
                    text_prompts=batch["switch_prompts2"]
                )
            
            # Get both switch indices for callback training
            switch_frame_index, callback_frame_index = self.model._get_callback_frame_indices(temp_max_length)
            
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                print(f"[SeqTrain-Trainer] switch_frame_index={switch_frame_index}, callback_frame_index={callback_frame_index}")
            
            # Store prompt strings for memory-aware training (will be set later when pipeline is initialized)
            # The inference_pipeline needs the actual text prompts (not just embeddings) for joint encoding
            self._pending_prompt_strings = {
                0: batch["switch_prompts0"][0] if isinstance(batch["switch_prompts0"], list) else batch["switch_prompts0"],
                1: batch["switch_prompts1"][0] if isinstance(batch["switch_prompts1"], list) else batch["switch_prompts1"],
                2: batch["switch_prompts2"][0] if isinstance(batch["switch_prompts2"], list) else batch["switch_prompts2"],
            }
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                print(f"[SeqTrain-Trainer] Stored prompt strings for memory-aware training")
            
            if (not dist.is_initialized() or dist.get_rank() == 0) and LOG_GPU_MEMORY:
                log_gpu_memory(f"streaming Training: After callback text encoding", device=self.device, rank=dist.get_rank())
        
        # Handle DMDSwitch (two prompts) - also handles DMDCallback in switch-only mode (no callback)
        elif isinstance(self.model, DMDSwitch) and "switch_prompts" in batch:
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
            ...
            # For DMDCallback in switch-only mode: set prompt_strings for memory-aware training
            if isinstance(self.model, DMDCallback):
                self._pending_prompt_strings = {
                    0: batch["prompts"][0] if isinstance(batch["prompts"], list) else batch["prompts"],
                    1: batch["switch_prompts"][0] if isinstance(batch["switch_prompts"], list) else batch["switch_prompts"],
                }
                if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                    print(f"[SeqTrain-Trainer] Stored prompt strings for memory-aware training (switch-only mode)")
            
            if (not dist.is_initialized() or dist.get_rank() == 0) and LOG_GPU_MEMORY:
                log_gpu_memory(f"streaming Training: After switch text encoding", device=self.device, rank=dist.get_rank())
        
        # Log selected prompts for reproducibility (rank 0 only, append mode)
        if not dist.is_initialized() or dist.get_rank() == 0:
            self._log_selected_prompts(batch, switch_frame_index)

        # Set up the sequence
        if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
            print(f"[SeqTrain-Trainer] Calling streaming_model.setup_sequence")
            
        # Get prompt strings for memory-aware training
        prompt_strings = getattr(self, '_pending_prompt_strings', None)
        ...
        self._pending_prompt_strings = None
    ...
    def _merge_lora_into_base(self, model, lora_state_dict: dict, scaling: float) -> int:
        """
        Merge LoRA weights into base model weights in-place.
        
        This "bakes in" the LoRA adaptations to the base model, so that
        a new LoRA can be trained on top without forgetting previous abilities.
        
        LoRA formula: W' = W + scaling * B @ A
        
        Args:
            model: The model to merge LoRA weights into
            lora_state_dict: LoRA state dict (from checkpoint)
            scaling: LoRA scaling factor (alpha / rank)
            
        Returns:
            Number of successfully merged weight pairs
        """
        # Get current model state dict
        model_state_dict = model.state_dict()
        
        # Find all LoRA A/B pairs
        lora_a_keys = sorted([k for k in lora_state_dict.keys() if "lora_A" in k])
        ...
        for lora_a_key in lora_a_keys:
            ...
            # Extract base model key from LoRA key
            # PEFT format: base_model.model.xxx.lora_A.default.weight
            base_key = lora_a_key
            for pattern in [".lora_A.default.weight", ".lora_A.weight",
                           "base_model.model.", "base_model."]:
                base_key = base_key.replace(pattern, "")
            if not base_key.endswith(".weight"):
                base_key = base_key + ".weight"
            
            # Try to find matching base weight
            found_base_key = None
            for key in [base_key, "model." + base_key, base_key.replace("model.", "")]:
            ...
            base_weight = model_state_dict[found_base_key]
            ...
            try:
                ...
                model_state_dict[found_base_key] = (base_weight.float() + delta).to(base_weight.dtype)
                merged_count += 1
    ...
    def _setup_visualizer(self):
        ...
        # Choose pipeline class depending on distribution_loss type and memory usage.
        # The maintained long-video path uses the EB memory pipeline; init training
        # keeps the plain causal visualization path.
        use_memory = getattr(self.config, "use_memory_in_training", False)
        if 'callback' in self.config.distribution_loss and use_memory:
            memory_encoder_config = {
                "pe_config": getattr(
                    self.config,
                    "memory_encoder_pe_config",
                    "PE-Core-B16-224",
                ),
                "checkpoint_path": getattr(self.config, "memory_encoder_pe_checkpoint", None),
            }
            self.vis_pipeline = InteractiveMemoryCausalInferencePipelineEB(
                args=self.config,
                device=self.device,
    ...
    def _visualize(self):
        ...
        # - ThreeTextDataset (dmd_callback): switch_prompts0/1/2
        # - TwoTextDataset (dmd_switch): prompts, switch_prompts  
        # - TextDataset: prompts
        if isinstance(self.vis_pipeline, InteractiveMemoryCausalInferencePipelineEB):
            if all(k in batch for k in ["switch_prompts0", "switch_prompts1", "switch_prompts2"]):
                text_prompts_list = [batch["switch_prompts0"], batch["switch_prompts1"], batch["switch_prompts2"]]
            elif "prompts" in batch and "switch_prompts" in batch:
                text_prompts_list = [batch["prompts"], batch["switch_prompts"]]
            else:
                print("[Warning] Unsupported batch format for callback visualization")
                return
        else:
            # For regular visualization, only need one prompt
            if "prompts" in batch:
                prompts = batch["prompts"]
            elif "switch_prompts0" in batch:
                # ThreeTextDataset format - use first prompt
                prompts = batch["switch_prompts0"]
            else:
                print("[Warning] Unsupported batch format for visualization")
                return
        ...
        for vid_len in self.vis_video_lengths:
            ...
            if isinstance(self.vis_pipeline, InteractiveMemoryCausalInferencePipelineEB):
                num_segments = len(text_prompts_list)
                if num_segments == 3:
                    if hasattr(self.model, "_get_callback_frame_indices"):
                        switch_frame_index, callback_frame_index = self.model._get_callback_frame_indices(vid_len)
                    else:
                        switch_frame_index = self._get_switch_frame_index(vid_len)
                        min_gap = getattr(self.config, "min_callback_gap", 24)
                        callback_choices = getattr(self.config, "callback_switch_choices", [])
                        valid_choices = [c for c in callback_choices if c >= switch_frame_index + min_gap and c < vid_len]
                        callback_frame_index = valid_choices[0] if len(valid_choices) > 0 else min(vid_len - 1, switch_frame_index + min_gap)
                    switch_frame_indices = [switch_frame_index, callback_frame_index]
                else:
                    switch_frame_index = self._get_switch_frame_index(vid_len)
                    switch_frame_indices = [switch_frame_index]

                videos = self.generate_video_with_multi_prompt_switch(
                    self.vis_pipeline,
                    vid_len,
                    text_prompts_list,
                    switch_frame_indices,
            ...
            for idx, video_np in enumerate(videos):
                if isinstance(self.vis_pipeline, InteractiveMemoryCausalInferencePipelineEB):
                    switch_suffix = "_".join(f"switch{i+1}_{fi}" for i, fi in enumerate(switch_frame_indices))
                    video_name = (
                        f"step_{self.step:07d}_rank_{dist.get_rank()}_sample_{idx}_len_{vid_len}{mode_info}"
                        f"_{switch_suffix}.mp4"
                    )
                else:

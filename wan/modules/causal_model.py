...
class CausalWanSelfAttention(nn.Module):
    ...
    def _gate_retrieval_kv(self, roped_query, k_local, v_local, retrieval_k, retrieval_v,
        ...
        is computed independently, then a bound decision is applied: all
        retrieved entries are kept only if every ratio stays at or below the
        threshold. Otherwise the layer falls back to local-window KV only,
        matching DySink.
        ...
        if gated:
            ...
            return (
                torch.cat([retrieval_k, k_local], dim=1),
                torch.cat([retrieval_v, v_local], dim=1),
            )
    ...
    def _gate_retrieval_kv_per_entry(self, roped_query, k_local, v_local,
        ...
        """
        Bound per-entry gating: each entry's anomaly_ratio is computed
        independently, but the decision is bound across all entries.
        If ANY single entry's anomaly_ratio exceeds the threshold, ALL
        retrieved entries are dropped and only local KV is used. Otherwise all
        retrieved entries are kept.
        """
        ...
        for i, k_entry in enumerate(k_entries):
            ...
            per_entry_ratios.append(ratio_val)
            self._gate_stats["anomaly_ratios"].append(ratio_val)

            if ratio_val > self.sink_anomaly_threshold:
                any_anomalous = True

        # DySink gate: max_e rho_e > tau_gate -> drop all retrieved KV.
        drop_all = any_anomalous

        for i, ratio_val in enumerate(per_entry_ratios):
            frame_idx = entry_frame_indices[i] if entry_frame_indices else i
            self._gate_stats["entry_details"].append(
                (frame_idx, bool(drop_all), ratio_val))

        self._gate_stats["calls"] += 1
        self._gate_stats["gated"] += int(drop_all)
        self._gate_stats["entries_total"] += n_entries
        self._gate_stats["entries_gated"] += n_entries if drop_all else 0

        if drop_all:
            return k_local, v_local
        else:
            return (
                torch.cat(list(k_entries) + [k_local], dim=1),
                torch.cat(list(v_entries) + [v_local], dim=1),
            )
        ...
        * Per-sample (correctness path): ``retrieval_kv_cache`` has
        ...
        if self.sink_anomaly_gate:
            ...
            k_cat, v_cat = self._gate_retrieval_kv(
                roped_query, k_cat, v_cat, retrieval_k, retrieval_v,
    ...
    def __init__(self,
        ...
        Args:
            ...
            sink_anomaly_gate (`bool`, *optional*, defaults to False):
                ...
                affinity. If any retrieved entry exceeds the threshold, the
                layer falls back to local-window KV only.

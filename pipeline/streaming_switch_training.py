...
class StreamingSwitchTrainingPipeline(StreamingTrainingPipeline):
    ...
    def _recache_after_switch(
        ...
        switch_recache_frames=None,
        local_attn_size_override=None,
    ):
        ...
        base_sw = self.local_attn_size if local_attn_size_override is None else local_attn_size_override
        sw = base_sw if base_sw > 0 else current_start_frame
        if switch_recache_frames is not None:
            frames_to_recache = torch.cat([switch_recache_frames, output], dim=1)[:, -sw:, ...]
        ...
        if num_recache_frames <= 0:
            if (not dist.is_initialized() or dist.get_rank() == 0) and DEBUG:
                print("[SeqTrain-DMDSwitch] Skip recache (num_recache_frames=0)")
            return

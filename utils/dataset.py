...
class ThreeTextDataset(Dataset):
    """Dataset that returns three text prompts per sample for multi-segment prompt-switch training.

    The dataset provides *three* prompts for each sample – typically used for videos
    with two switch points: the first prompt for the initial segment, the second prompt
    after the first switch, and the third prompt after the second switch.
    ...
    Args:
        ...
        switch_prompt_path (str): Path to a text file containing the *second*
            prompt for each sample. Must have the **same number of lines** as
            ``prompt_path`` so that prompts are paired 1-to-1.
        third_prompt_path (str): Path to a text file containing the *third*
            prompt for each sample. Must have the **same number of lines** as
            ``prompt_path`` so that prompts are paired 1-to-1.
    """
    def __init__(self, prompt_path: str, switch_prompt_path: str, third_prompt_path: str):
        # Load the first-segment prompts.
        with open(prompt_path, encoding="utf-8") as f:
            self.prompt_list = [line.rstrip() for line in f]
        ...
        # Load the third-segment prompts.
        with open(third_prompt_path, encoding="utf-8") as f:
            self.third_prompt_list = [line.rstrip() for line in f]

        assert len(self.switch_prompt_list) == len(self.prompt_list), (
            "The first and second prompt files must contain the same number of lines so that "
            "each first-segment prompt is paired with exactly one second-segment prompt."
        )
        assert len(self.third_prompt_list) == len(self.prompt_list), (
            "The first and third prompt files must contain the same number of lines so that "
            "each first-segment prompt is paired with exactly one third-segment prompt."
        )
    ...
    def __getitem__(self, idx):
        return {
            "switch_prompts0": self.prompt_list[idx],              # first-segment prompt
            "switch_prompts1": self.switch_prompt_list[idx],       # second-segment prompt
            "switch_prompts2": self.third_prompt_list[idx],        # third-segment prompt
            "idx": idx,
        }

...
class WanTextEncoder(torch.nn.Module):
    def __init__(self) -> None:
        ...
        self.text_encoder.load_state_dict(
            torch.load("/c20250508/241406048/wan2.1-1.3B/models_t5_umt5-xxl-enc-bf16.pth",
        ...
        self.tokenizer = HuggingfaceTokenizer(
            name="/c20250508/241406048/wan2.1-1.3B/google/umt5-xxl/", seq_len=512, clean='whitespace')
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self):
        ...
        self.model = _video_vae(
            pretrained_path="/c20250508/241406048/wan2.1-1.3B/Wan2.1_VAE.pth",
            z_dim=16,
        ).eval().requires_grad_(False)
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        if is_causal:
            extra_kwargs = {}
            if sink_anomaly_gate:
                extra_kwargs.update(
                    sink_anomaly_gate=True,
                    sink_anomaly_threshold=sink_anomaly_threshold,
                )
            self.model = CausalWanModel.from_pretrained(
                f"/c20250508/241406048/{model_name}/",
                local_attn_size=local_attn_size,
                sink_size=sink_size,
                **extra_kwargs)
        else:
            self.model = WanModel.from_pretrained(f"/c20250508/241406048/{model_name}/")
    ...
    def forward(
        ...
        retrieval_kv_cache: Optional[List[dict]] = None,

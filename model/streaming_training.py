...
class StreamingTrainingModel:
    ...
    def reset_state(self):
        ...
        self.state = {
            ...
            "has_switched": False,  # Track whether first prompt switch has occurred
            "has_callback": False,  # Track whether callback switch has occurred
    ...
    def _should_callback(self, chunk_start_frame: int, chunk_size: int) -> bool:
        """Determine whether to trigger callback switch (DMDCallback only)"""
        from pipeline.streaming_callback_training import StreamingCallbackTrainingPipeline
        if not isinstance(self.inference_pipeline, StreamingCallbackTrainingPipeline):
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                print(f"[StreamingTrain-Model] Not a callback pipeline, no callback")
            return False
        
        # If already had callback, do not callback again
        if self.state.get("has_callback", False):
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                print(f"[StreamingTrain-Model] Already had callback, not triggering again")
            return False
        
        # Must have switched first before callback
        if not self.state.get("has_switched", False):
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                print(f"[StreamingTrain-Model] Not switched yet, cannot callback")
            return False
        
        callback_info = self.state["conditional_info"].get("callback_info", {})
        callback_frame_index = callback_info.get("callback_frame_index")
        
        if callback_frame_index is None:
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                print(f"[StreamingTrain-Model] No callback_frame_index, not triggering callback")
            return False
        
        # Check if the callback point falls within the current chunk range
        chunk_end_frame = chunk_start_frame + chunk_size
        should_callback = chunk_start_frame <= callback_frame_index < chunk_end_frame
        
        if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
            print(f"[StreamingTrain-Model] Callback check: callback_frame={callback_frame_index}, chunk=[{chunk_start_frame}, {chunk_end_frame}), should_callback={should_callback}")
    ...
    def _get_current_conditional_dict(self, chunk_start_frame: int) -> dict:
        ...
        # Check for callback first (takes priority over switch)
        callback_info = cond_info.get("callback_info", {})
        if callback_info:
            callback_frame_index = callback_info.get("callback_frame_index")
            if callback_frame_index is not None:
                if self.state.get("has_callback", False) or chunk_start_frame >= callback_frame_index:
                    # If already had callback, or current frame has reached the callback point, use callback prompt
                    if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                        print(f"[StreamingTrain-Model] Using callback conditional_dict for chunk starting at frame {chunk_start_frame}")
                    return callback_info.get("callback_conditional_dict", cond_info["conditional_dict"])
    ...
    def _generate_chunk(
        ...
        # NOTE: do NOT update has_switched/has_callback before generation.
        # Otherwise callback pipeline sees the segment as already switched and
        # skips the in-chunk recache path.
        ...
        if isinstance(self.inference_pipeline, StreamingSwitchTrainingPipeline):
            ...
            # Handle callback logic for DMDCallback models (three-segment training)
            callback_info = self.state["conditional_info"].get("callback_info", {})
            if callback_info and self._should_callback(chunk_start_frame, noise_chunk.shape[1]):
                if (not dist.is_initialized() or dist.get_rank() == 0):
                    print(f"[StreamingTrain-Model] Callback switch at frame {callback_info['callback_frame_index']}")
                
                # Compute the relative callback frame index
                relative_callback_index = max(0, callback_info["callback_frame_index"] - chunk_start_frame)
                kwargs["callback_frame_index"] = relative_callback_index
                kwargs["callback_conditional_dict"] = callback_info["callback_conditional_dict"]
                
                # Pass previous_frames for callback recache
                if self.state["previous_frames"] is not None:
                    kwargs["callback_recache_frames"] = self.state["previous_frames"]
                    
                    if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                        print(f"[StreamingTrain-Model] Passed previous_frames for callback recache: {self.state['previous_frames'].shape}")
                
                if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                    print(f"[StreamingTrain-Model] Adding callback parameters: relative_callback_index={relative_callback_index}")
                
                triggered_callback = True
        
        # Sync the persistent segment state to the pipeline so that chunks
        # starting *after* a prompt switch still use the correct text features
        # for memory retrieval and storage.
        if isinstance(self.inference_pipeline, StreamingCallbackTrainingPipeline):
            if self.state.get("has_callback", False):
                self.inference_pipeline.set_current_segment(2)
            elif self.state.get("has_switched", False):
                self.inference_pipeline.set_current_segment(1)
            else:
                self.inference_pipeline.set_current_segment(0)
        ...
        # Mark switch/callback completion AFTER current chunk generation so the
        # pipeline can execute in-chunk recache exactly at switch frames.
        if triggered_switch:
            self.state["has_switched"] = True
        if triggered_callback:
            self.state["has_callback"] = True
        # if DEBUG:
    ...
    def setup_sequence(
        ...
        prompt_strings: Optional[Dict[int, str]] = None,  # {0: prompt1, 1: prompt2, 2: prompt3}
    ):
        ...
        if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
            ...
            print(f"[StreamingTrain-Model] callback_frame_index={callback_frame_index}")
        ...
        # DMDCallback related information (three-segment training)
        if callback_conditional_dict is not None and callback_frame_index is not None:
            self.state["conditional_info"]["callback_info"] = {
                "callback_conditional_dict": callback_conditional_dict,
                "callback_frame_index": callback_frame_index,
            }
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                print(f"[StreamingTrain-Model] DMDCallback info saved: callback_frame_index={callback_frame_index}")
        
        # Set prompt strings for memory-aware training (joint frame+prompt encoding)
        if prompt_strings is not None and hasattr(self.inference_pipeline, 'set_current_prompts'):
            ...
            if DEBUG and (not dist.is_initialized() or dist.get_rank() == 0):
                print(f"[StreamingTrain-Model] Set prompt strings for memory-aware training: {len(prompt_strings)} segments")
